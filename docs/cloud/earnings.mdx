...
View balances, hosting spend, and redemptions at [Cloud → Monetization](https://cloud.eliza.app/cloud/monetization).

```bash
curl "https://api.eliza.app/api/v1/redemptions/balance" \
...
```bash
curl -X POST "https://api.eliza.app/api/v1/redemptions" \
  -H "Authorization: Bearer $ELIZA_CLOUD_API_KEY" \
...
Send `asset: "eliza"` explicitly for this token-quote flow. Legacy raw requests that omit `asset` default to `usdc`; do not rely on that fallback when pairing a quote and redemption. Supported networks are `ethereum`, `base`, `bnb`, and `solana`, subject to payout availability.
...
```bash
curl "https://api.eliza.app/api/v1/apps/$APP_ID/earnings?days=30" \
  -H "Authorization: Bearer $ELIZA_CLOUD_API_KEY"
```
