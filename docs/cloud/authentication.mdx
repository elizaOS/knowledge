...
1. Navigate to [Cloud → API Keys](https://cloud.eliza.app/cloud/api-keys)
...
```bash
curl -X GET "https://api.eliza.app/api/v1/dashboard" \
...
```javascript
const response = await fetch('https://api.eliza.app/api/v1/dashboard', {
  headers: {
...
'https://api.eliza.app/api/v1/dashboard',
...
API keys authenticate product APIs, but they cannot list, create, update,
regenerate, or revoke API keys. Key-management routes require an authenticated
Steward browser session so one leaked or lower-privilege key cannot take over
other credentials in the organization.

Use [Cloud → API Keys](https://cloud.eliza.app/cloud/api-keys) to manage
keys. Browser clients may call the same routes with the Steward session cookie
by setting `credentials: "include"`; an `Authorization: Bearer eliza_…` header
returns `401` on every key-management route.
...
3. **POST /api/auth/siwe/verify** — Body `{ message, signature }`. Server validates domain and signature, consumes the nonce, then finds or creates a zero-balance user/org and returns **`apiKey`** with `user` and `organization`. Credits are added only through explicit funding or promotion-code redemption.
...
- Without wallet sig: send **body.walletAddress**; that wallet’s org is credited. New wallets start at zero (payment only).
...
```bash
curl -X POST "https://api.eliza.app/api/v1/chat/completions" \
