...
elizaOS Cloud bills organization balances in USD-denominated cloud credits for
flexible, pay-as-you-go pricing. One cloud credit equals **$1.00 USD**; balances
and checkout amounts use that same denomination.
...
- **Credits**: USD-denominated balance units (1 cloud credit = $1 USD)
...
View your balance at [Cloud → Billing](https://cloud.eliza.app/cloud/billing).

### API

```bash
curl -X GET "https://api.eliza.app/api/v1/credits/summary" \
...
```bash
curl -X POST "https://api.eliza.app/api/v1/credits/checkout" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
credits granted. The deprecated `credits` request field remains a 1:1
compatibility alias.
...
```bash
curl -X POST "https://api.eliza.app/api/v1/topup/10" \
...
```bash
curl -X PUT "https://api.eliza.app/api/v1/billing/settings" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "autoTopUp": {
...
```bash
curl -X GET "https://api.eliza.app/api/invoices/list" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
<Info>
  ...
  Billing](https://cloud.eliza.app/cloud/billing) settings.
</Info>

## Cost Alerts

Cost alerts can be configured in your [Cloud → Billing](https://cloud.eliza.app/cloud/billing) settings. You can set:

- **Email notifications** when balance drops below a threshold
- **Auto top-up** to automatically add credits when balance is low

## Shared and Paid Access

New accounts start with:

- A `$0` paid-credit balance
- Shared personal Eliza chat without paid compute
- Community support

Purchased top-ups, promotion-code credits, and referral credits are added only
through their explicit ledger flows. Dedicated agents and other paid compute
require a positive balance and an explicit activation.
