...
Browse and connect MCP servers at [Cloud → MCPs](https://cloud.eliza.app/cloud/mcps).

### API

```bash
curl -X POST "https://api.eliza.app/api/v1/mcps" \
...
```bash
curl -X GET "https://api.eliza.app/api/mcp/registry" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://api.eliza.app/api/mcp" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
```bash
curl -X POST "https://api.eliza.app/api/mcps/weather" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
```bash
curl -X GET "https://api.eliza.app/api/v1/mcps?agentId=agent_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X DELETE "https://api.eliza.app/api/v1/mcps/mcp_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://api.eliza.app/api/v1/mcps/{mcpId}/publish" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "visibility": "public",
...
- **Monitor Usage** — Track tool invocations in [Cloud](/cloud) for optimization
