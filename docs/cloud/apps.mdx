...
```bash
curl -X POST "https://api.eliza.app/api/v1/apps" \
...
```bash
curl -X PUT "https://api.eliza.app/api/v1/apps/$APP_ID/monetization" \
  -H "Authorization: Bearer $ELIZA_CLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH "https://api.eliza.app/api/v1/apps/$APP_ID" \
  -H "Authorization: Bearer $ELIZA_CLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```ts
const response = await fetch(`https://api.eliza.app/api/v1/apps/${APP_ID}/chat`, {
  method: "POST",
  headers: {
...
Redeem available earnings for elizaOS tokens from [Cloud → Monetization](https://cloud.eliza.app/cloud/monetization) or the [Redemptions API](/cloud/api/redemptions).
...
- Handle 402 responses by sending users to [Cloud → Billing](https://cloud.eliza.app/cloud/billing).
