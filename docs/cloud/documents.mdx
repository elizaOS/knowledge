...
```bash
# Upload a document
curl -X POST "https://api.eliza.app/api/v1/documents/upload-file" \
...
const response = await fetch('https://api.eliza.app/api/v1/documents/upload-file', {
method: 'POST',
headers: {
...
with open('document.pdf', 'rb') as f:
    response = requests.post(
        'https://api.eliza.app/api/v1/documents/upload-file',
...
```bash
curl -X GET "https://api.eliza.app/api/v1/documents/check?documentId=doc_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://api.eliza.app/api/v1/documents/query" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```bash
curl -X GET "https://api.eliza.app/api/v1/documents?agentId=agent_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://api.eliza.app/api/v1/documents/doc_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X DELETE "https://api.eliza.app/api/v1/documents/doc_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```javascript
// 1. Query knowledge
const knowledge = await fetch("https://api.eliza.app/api/v1/documents/query", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
