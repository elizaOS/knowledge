...
```bash
curl -X POST "https://api.eliza.app/api/v1/generate-image" \
...
```javascript
const response = await fetch('https://api.eliza.app/api/v1/generate-image', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.eliza.app/api/v1/generate-image',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json',
    },
