...
```bash
curl -X POST "https://api.eliza.app/api/v1/embeddings" \
...
```javascript
const response = await fetch('https://api.eliza.app/api/v1/embeddings', {
  method: 'POST',
  headers: {
...
```javascript
// Embed your query
const queryResponse = await fetch("https://api.eliza.app/api/v1/embeddings", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
