...
```bash
curl -X POST "https://api.eliza.app/api/v1/voice/tts" \
...
```javascript
const response = await fetch('https://api.eliza.app/api/v1/voice/tts', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.eliza.app/api/v1/voice/tts',
    headers={
        'X-API-Key': 'YOUR_API_KEY',
        'Content-Type': 'application/json',
    },
...
```bash
curl -X POST "https://api.eliza.app/api/v1/voice/stt" \
  -H "X-API-Key: YOUR_API_KEY" \
...
```bash
curl -X POST "https://api.eliza.app/api/v1/voice/clone" \
  -H "X-API-Key: YOUR_API_KEY" \
  -F "name=My Voice" \
  -F "cloneType=instant" \
  -F "file0=@sample1.mp3" \
...
```bash
curl "https://api.eliza.app/api/v1/voice/123e4567-e89b-12d3-a456-426614174000" \
  -H "X-API-Key: YOUR_API_KEY"
```
...
```bash
curl -X PATCH "https://api.eliza.app/api/v1/voice/123e4567-e89b-12d3-a456-426614174000" \
  -H "X-API-Key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE "https://api.eliza.app/api/v1/voice/123e4567-e89b-12d3-a456-426614174000" \
  -H "X-API-Key: YOUR_API_KEY"
```
