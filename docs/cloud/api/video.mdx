...
```bash
curl -X POST "https://api.eliza.app/api/v1/generate-video" \
...
```javascript
const response = await fetch('https://api.eliza.app/api/v1/generate-video', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.eliza.app/api/v1/generate-video',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json',
    },
