...
- **Wildcard Subdomains**: Automatic `*.apps.eliza.app` subdomains for every app
...
Navigate to [Cloud → Apps](/cloud/apps) and select an app to manage its domains.

### API

```bash
# Add a custom domain to your app
curl -X POST "https://api.eliza.app/api/v1/apps/{appId}/domains" \
...
When you deploy an app, it automatically receives a subdomain like `myapp.apps.eliza.app`.
...
Every deployed app automatically gets a subdomain under `apps.eliza.app`:

```
https://myapp.apps.eliza.app
```
...
```bash
curl -X POST "https://api.eliza.app/api/v1/apps/{appId}/domains/verify" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "domain": "myapp.example.com"
...
```bash
curl -X POST "https://api.eliza.app/api/v1/apps/{appId}/domains/status" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "domain": "myapp.example.com"
...
```bash
curl -X POST "https://api.eliza.app/api/v1/apps/{appId}/domains/sync" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
  ...
  "domains": [
    {
      "id": "dom_abc123",
      "subdomain": "myapp",
      "subdomainUrl": "https://myapp.apps.eliza.app",
...
```bash
curl -X DELETE "https://api.eliza.app/api/v1/apps/{appId}/domains" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "domain": "myapp.example.com"
...
<Warning>
  ...
  that URL. The subdomain at `*.apps.eliza.app` remains active.
</Warning>
...
- **Use Subdomains First** — Test with `*.apps.eliza.app` before adding custom domains
