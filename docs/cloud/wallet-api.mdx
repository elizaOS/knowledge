...
- **SIWE**: `GET /api/auth/siwe/nonce` → sign message → `POST /api/auth/siwe/verify` → receive `apiKey`, `user`, `organization`. New accounts start at zero; funding is explicit.
...
- Without: send **body.walletAddress**; that wallet’s org is credited. New wallets created here start at zero.
