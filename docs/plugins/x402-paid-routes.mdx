...
Verification uses public RPC defaults; override per network via runtime settings **`BASE_RPC_URL`**, **`POLYGON_RPC_URL`**, **`ETHEREUM_RPC_URL`** (see `getSetting` in the agent runtime).
