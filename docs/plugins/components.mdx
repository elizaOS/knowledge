...
Evaluators are registered post-turn items that contribute context, schema, prompt instructions, and processors to the unified evaluator service.

### Evaluator Interface

Evaluators process and extract information from the just-finished turn. Each evaluator has:
...
- **providers**: Context providers to include in the shared prompt
- **schema**: JSON schema for this evaluator's top-level output section
- **shouldRun**: Function to determine if evaluator should be active this turn
- **prepare**: Optional data loader for evaluator-specific prompt context
- **prompt**: Instructions for this evaluator's output section
- **processors**: Post-model processors that persist or act on extracted data

Active evaluators run after the agent response in one structured model call. Each evaluator receives its own output section, then its processors run in priority order.
...
| --------------------------- | ---------------------- | ------------------------------ |
| `factMemoryEvaluator`       | Durable fact memory    | Fact reconciliation operations |
| `relationshipEvaluator`     | Relationship updates   | Entity relationship updates    |
| `identityEvaluator`         | Platform identities    | Handles and aliases            |
| `successEvaluator`          | Turn completion status | Task completion state          |
| `summaryEvaluator`          | Context compression    | Rolling conversation summary   |
| `longTermMemoryEvaluator`   | Long-term memory       | Durable memory items           |
| `experiencePatternEvaluator`| Experience learning    | Novel lessons from the room    |
| `skillProposalEvaluator`    | Skill creation         | Proposed reusable skills       |
| `skillRefinementEvaluator`  | Skill improvement      | Updates for existing skills    |

### Evaluator Flow

```mermaid
flowchart TD
    Response[Agent Response] --> Validate[shouldRun]
    Validate -->|active| Compose[Compose shared providers]
    Validate -->|inactive| Skip[Skip]
    Compose --> Model[Single structured evaluator call]
    Model --> Process[Run processors]
    Process --> Store[Store or apply results]
    ...
    class Compose,Model,Process processing
    class Store storage
