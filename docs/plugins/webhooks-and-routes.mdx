...
For **HTTP 402 micropayments (x402)** on plugin routes (paid APIs, wallet flows, test mode, and buyer clients), see [x402 paid plugin routes](/plugins/x402-paid-routes) and the [x402 roadmap](/plugins/x402-roadmap) for protocol alignment and planned work.
...
Use generated package scripts to run your webhook tests:

```bash
# Run all tests
bun run test

# Run component or e2e scripts when your package defines them
