...
`ELIZA_CONFIG_PATH` takes precedence over `ELIZA_STATE_DIR`. Both support `~` expansion for the home directory. Branded builds may also provide `MILADY_STATE_DIR` and `MILADY_CONFIG_PATH` as aliases.
