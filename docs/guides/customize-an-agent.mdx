...
<Note>
  The `elizaos` CLI creates and upgrades workspaces. Running the app is handled
  by generated package scripts such as `bun run dev`.
</Note>
...
<Note>
  To use a different port, use the port option or environment variable supported
  by your generated project script.
</Note>
...
To add large amounts of knowledge to your agent, check out [knowledge](/plugin-registry/documents) which can ingest almost any type of file or media including PDFs, Word docs, markdown, text files, JSON, CSV, XML, and more. It can handle entire document collections, websites, and knowledge bases.
