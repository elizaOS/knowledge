...
- inbox, calendar, dossier, cross-channel search, and approval code under `eliza/plugins/app-lifeops/src`

### TODOs

- [ ] Add a new strict live E2E file or strict section in the existing assistant-user-journeys suite.
- [ ] Extend `seedGoogleConnector` in `eliza/plugins/app-lifeops/test/assistant-user-journeys.live.e2e.test.ts` to grant Drive read scope in addition to Gmail/Calendar.
...
- `eliza/plugins/app-lifeops/test/travel-duffel.integration.test.ts`
- calendar, approval queue, and travel adapter code under `eliza/plugins/app-lifeops/src`
...
- browser/computer-use/autofill/approval code under `eliza/plugins/app-lifeops/src`
...
- [ ] Split `postLiveConversationMessage()` in `eliza/plugins/app-lifeops/test/helpers/lifeops-live-harness.ts` into a strict single-attempt path and a named recovery path.
...
- `eliza/plugins/app-lifeops/test/lifeops-connector-certification.contract.test.ts`
- connector status/auth code under `eliza/plugins/app-lifeops/src`
...
- `eliza/plugins/app-lifeops/test/background-job-parity.contract.test.ts`
- `test/scenarios/lifeops.workflow-events`
- trigger/worker code under `eliza/plugins/app-lifeops/src`
...
- `eliza/plugins/app-lifeops/test/assistant-user-journeys.live.e2e.test.ts`
- identity/search/memory code under `eliza/plugins/app-lifeops/src` and `eliza/packages/agent`

### TODOs

- [ ] Add a deterministic canonical-identity fixture helper under `eliza/plugins/app-lifeops/test/helpers` with stable UUIDs and explicit Gmail, Signal, Telegram, and WhatsApp identities for one person.
...
- [ ] Add one strict live merge case to `eliza/plugins/app-lifeops/test/assistant-user-journeys.live.e2e.test.ts` that first surfaces the four identities, then asks the assistant to unify them, then asserts the graph resolves to one canonical person with four member identities.
...
- [ ] Add deterministic graph-level coverage in `eliza/plugins/app-lifeops/test/relationships.real.test.ts` or a nearby real test so `getGraphSnapshot()` and `getPersonDetail()` are asserted directly.
...
- [ ] If the UI must collapse the Rolodex into one visible person row, add a canonical-identity bridge in `eliza/plugins/app-lifeops/src/lifeops/schema.ts` and `repository.ts` because the current `(agentId, primaryChannel, primaryHandle)` uniqueness model cannot represent one merged four-platform contact cleanly.
...
- follow-up tracker, relationships, Gmail, and cross-channel send code under `eliza/plugins/app-lifeops/src`
...
- `eliza/plugins/app-lifeops/test/notifications-push.integration.test.ts`
- `eliza/plugins/app-lifeops/test/intent-sync.real.test.ts`
...
- [ ] Extend `eliza/plugins/app-lifeops/test/intent-sync.real.test.ts` with an explicit acknowledge subaction case and a stronger assertion that previously pending intents disappear after acknowledgement.
- [ ] Extend `eliza/plugins/app-lifeops/test/device-bus.test.ts` with the real remote-delivery happy path, not only the local fallback path, and assert `deliveredTo` matches the desktop/mobile ladder routing.
- [ ] Keep `eliza/plugins/app-lifeops/test/notifications-push.integration.test.ts` as the transport gate and run it against a real Ntfy backend or equivalent topic so the live ladder uses a real push transport.
