...
**Phase 1 done:**
- App-side calendar discovery, include preferences, merged feed aggregation, settings UI, and calendar-origin labels are implemented in `eliza/plugins/app-lifeops`.
...
- Verification completed:
  - `bun run --cwd eliza/plugins/app-lifeops test -- service-mixin-calendar.test.ts`
...
- Contracts barrel: `eliza/plugins/app-lifeops/src/contracts/index.ts` re-exports from `@elizaos/shared/contracts/lifeops`, so adding to shared contracts file is sufficient.
...
- [x] Add `listGoogleCalendars({ accessToken })` to [eliza/plugins/app-lifeops/src/lifeops/google-calendar.ts](eliza/plugins/app-lifeops/src/lifeops/google-calendar.ts)
...
- [x] `listCalendars` method on `LifeOpsCalendarService` in [eliza/plugins/app-lifeops/src/lifeops/service-mixin-calendar.ts](eliza/plugins/app-lifeops/src/lifeops/service-mixin-calendar.ts)
- [x] Expose `GET /api/lifeops/calendar/calendars` route in [eliza/plugins/app-lifeops/src/routes/lifeops-routes.ts](eliza/plugins/app-lifeops/src/routes/lifeops-routes.ts) returning `{ calendars: LifeOpsCalendarSummary[] }`
...
- [x] Extend the lifeops preferences store (look for existing preferences persistence under `eliza/plugins/app-lifeops/src/lifeops/repository.ts`) with a `calendarFeedIncludes: Record<calendarId, boolean>` field. Default `true` for every calendar the user has.
...
- [x] Modify `getCalendarFeed` in [eliza/plugins/app-lifeops/src/lifeops/service-mixin-calendar.ts](eliza/plugins/app-lifeops/src/lifeops/service-mixin-calendar.ts) to:
...
- [x] [eliza/plugins/app-lifeops/src/components/chat/widgets/plugins/lifeops-channels.tsx](eliza/plugins/app-lifeops/src/components/chat/widgets/plugins/lifeops-channels.tsx) — no change to the fetch call. The backend already aggregates.
...
- [x] Audit every lifeops calendar action in [eliza/plugins/app-lifeops/src/actions/](eliza/plugins/app-lifeops/src/actions/) — confirm they read across all authorized calendars, not just the feed-included ones.
...
- [ ] New file: `eliza/plugins/app-lifeops/src/lifeops/briefing-composer.ts`
...
- [ ] New action `COMPOSE_BRIEFING` in `eliza/plugins/app-lifeops/src/actions/briefing.ts`
...
- [ ] Apply the existing privacy filter (`eliza/plugins/app-training/src/core/privacy-filter.ts`) before any briefing content lands in a trajectory.
...
- [ ] One end-to-end test per phase landing in `eliza/plugins/app-lifeops/test/` (look at the existing `.real.test.ts` pattern).
